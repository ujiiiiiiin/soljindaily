...
function DdayCard({ dday, index, onClick }) {
  return (
    <button onClick={onClick} className="cs-card" style={{
      minWidth: 188, padding: '18px 18px 16px', flexShrink: 0, textAlign: 'left',
      borderRadius: 16, border: '1px solid #E1EAF5', background: '#FFFFFF',
      boxShadow: '0 3px 12px rgba(31,41,55,0.06)'
    }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#64748B', marginBottom: 8 }}>D-DAY</div>
      <div style={{ fontSize: 15, fontWeight: 700, color: '#1F2937', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{dday.title}</div>
      <div style={{ fontSize: 28, lineHeight: 1.15, fontWeight: 800, color: '#2F80ED', marginTop: 8 }}>{calcDday(dday.date)}</div>
      <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 5 }}>{dday.date.replace(/-/g, '.')}</div>
    </button>
  );
}
...
function scheduleDateLabel(schedule) {
  if (schedule.endDate && schedule.endDate !== schedule.date) {
    return `${formatDisplayDate(schedule.date)} ~ ${formatDisplayDate(schedule.endDate)}`;
  }
  return formatDisplayDate(schedule.date);
}
  ...
  return (
    <div onClick={onClick} className="cs-card" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 14px', marginBottom: 8, cursor: 'pointer' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, marginBottom: 3, minWidth: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#2F80ED', whiteSpace: 'nowrap', flexShrink: 0 }}>
  ...
  const upcoming = useMemo(() => {
    ...
    return [...schedules]
      .filter((s) => {
        ...
        return s.date <= todayStr && end >= todayStr || s.date >= todayStr;
      })
...
function PhotoFrame({ photo, index, onClick }) {
  return (
    <button onClick={onClick} style={{
      width: 'calc((100% - 16px) / 3)', aspectRatio: '1', padding: 0, overflow: 'hidden',
      borderRadius: 12, border: 'none', background: '#F1F5F9', boxShadow: 'none'
    }}>
      <img src={photo.dataUrl} alt={photo.caption || ''} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
    </button>
  );
}
    ...
    <div>
      <section style={{ paddingTop: 6 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h3 className="cs-serif" style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>D-DAY</h3>
          <button onClick={onAddDday} style={{
            display: 'flex', alignItems: 'center', gap: 4, padding: '7px 10px',
            borderRadius: 9, border: '1px solid #D8E5F5', background: '#FFFFFF',
            color: '#2F80ED', fontSize: 12, fontWeight: 700
          }}>
            <Plus size={14} /> 추가
          </button>
        </div>
        <div className="cs-scroll" style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 5 }}>
          {ddays.length === 0 ? (
            <button onClick={onAddDday} className="cs-card" style={{
              width: '100%', padding: '22px 18px', borderRadius: 16, border: '1px solid #E1EAF5',
              background: '#FFFFFF', color: '#94A3B8', textAlign: 'left'
            }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#64748B' }}>기념일이나 중요한 날짜를 등록해보세요</div>
              <div style={{ fontSize: 11, marginTop: 5 }}>예: 결혼기념일 · 집 마련 목표일</div>
            </button>
          ) : (
            <>
              {ddays.map((d, i) => (
      ...
      <section style={{ marginTop: 28, marginBottom: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}>
          ...
          <button onClick={onAddPhoto} style={{ fontSize: 12, color: '#2F80ED', display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', fontWeight: 700 }}>
            <Camera size={14} /> 사진 추가
          </button>
        </div>
        {photos.length === 0 ? (
          <button onClick={onAddPhoto} className="cs-card" style={{
            width: '100%', minHeight: 132, padding: '28px 0', borderRadius: 14,
            border: '1px solid #E1EAF5', color: '#94A3B8', background: '#FFFFFF',
            textAlign: 'center'
          }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: '#F1F6FD', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 10px', color: '#8AA8CB' }}>
              <Camera size={21} />
            </div>
            <div style={{ width: '100%', textAlign: 'center', fontSize: 13, fontWeight: 600, color: '#64748B' }}>앨범에 첫 사진을 추가해보세요</div>
          </button>
        ) : (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, padding: 0 }}>
...
function ScheduleModal({ initial, defaultDate, settings, onClose, onSave, onDelete }) {
  ...
  function handleSubmit() {
    ...
    const safeEndDate = endDate && endDate >= date ? endDate : date;
  ...
  return (
    <ModalWrapper title={initial ? '일정 수정' : '일정 추가'} onClose={onClose}>
      ...
      <div style={{ marginBottom: 14 }}>
        <label className="cs-label">일정 날짜</label>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr', alignItems: 'center', gap: 8 }}>
          <div>
            <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 5 }}>시작일</div>
            <input
              type="date"
              className="cs-input"
              value={date}
          ...
          <div style={{ textAlign: 'center', color: '#94A3B8', paddingTop: 17 }}>~</div>
          <div>
            <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 5 }}>종료일</div>
            <input
              type="date"
              className="cs-input"
              value={endDate}
...
function DdayModal({ initial, onClose, onSave, onDelete }) {
  ...
  function handleSubmit() {
    ...
    onSave({ id: initial?.id || genId('d'), title: title.trim(), date, createdAt: initial?.createdAt || Date.now() });
  }
    ...
    <div style={{ minHeight: '100vh', background: '#F7F9FC', display: 'flex', justifyContent: 'center' }}>
      <div className="cs-app" style={{ width: '100%', maxWidth: 480, height: '100vh', position: 'relative', display: 'flex', flexDirection: 'column', background: '#F7F9FC', overflow: 'hidden' }}>
        ...
        <button onClick={() => openAddSchedule()} style={{
          ...
          boxShadow: '0 8px 20px rgba(47,128,237,0.28)', border: 'none', zIndex: 40,
        }}>
          <Plus size={26} />
        </button>
